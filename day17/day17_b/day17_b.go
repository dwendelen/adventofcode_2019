...
func main() {
	...
	robotInput1 := "A,B,A,C,A,B,A,C,B,C\n"
	robotInput2 := "R,4,L,12,L,8,R,4\n"
	robotInput3 := "L,8,R,10,R,10,R,6\n"
	robotInput4 := "R,4,R,10,L,12\n"
	robotInput5 := "n\n"
	...
	printOutput(output)
	doInput(input, robotInput1)
	printOutput(output)
	doInput(input, robotInput2)
	printOutput(output)
	doInput(input, robotInput3)
	printOutput(output)
	doInput(input, robotInput4)
	printOutput(output)
	doInput(input, robotInput5)
	printOutput(output)
...
func printOutput(output chan int64) {
	fmt.Print(">>> ")
	for {
		out, more := <-output
		if !more {
...
func doInput(input chan int64, toInput string) {
	fmt.Print("<<< ", toInput)
	for _, char := range toInput {
		input <- int64(char)
	}
}
